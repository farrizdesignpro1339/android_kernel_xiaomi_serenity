...
bad_version:
	pr_warn("%s: disagrees about version of symbol %s (serenity: allowed)\n",
	       info->name, symname);
	/* serenity: stock vendor_dlkm modules are reused as-is; accept CRC
	 * drift against this rebuilt kernel instead of rejecting them. */
	return 1;
}
