		 ...
		 uint8_t interface)
{
	...
	if (!np) {
        ...
        ret = of_property_read_string(np,"compatible",&nfcinfo_name);
        pr_err("%s: chip type %s\n", __func__,nfcinfo_name);
        if(0 == ret){
            get_hardware_info_data(HWID_NFC,nfcinfo_name);
        }else{
            pr_err("%s: read hardware_info_data fail\n", __func__);
        }
