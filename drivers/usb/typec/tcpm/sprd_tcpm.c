...
#ifdef SERENITY_FEATURE_CHG_BASIC
ATOMIC_NOTIFIER_HEAD(serenity_chg_usb_tcpm_event_notifier);
EXPORT_SYMBOL_GPL(serenity_chg_usb_tcpm_event_notifier);
int serenity_chg_usb_tcpm_reg_event_notifier(struct notifier_block *nb)
{
	return atomic_notifier_chain_register(&serenity_chg_usb_tcpm_event_notifier, nb);
}
EXPORT_SYMBOL_GPL(serenity_chg_usb_tcpm_reg_event_notifier);

void serenity_chg_usb_tcpm_unreg_event_notifier(struct notifier_block *nb)
{
	atomic_notifier_chain_unregister(&serenity_chg_usb_tcpm_event_notifier, nb);
}
EXPORT_SYMBOL_GPL(serenity_chg_usb_tcpm_unreg_event_notifier);
#endif
				       ...
				       const __le32 *payload, int cnt)
{
	...
	if (port->svid_header == 0) {
		...
		atomic_notifier_call_chain(&serenity_chg_usb_tcpm_event_notifier, SPRD_CMD_DISCOVER_IDENT, &port->svid_header);
	}
#endif
}
				    ...
				    const __le32 *payload, int cnt)
{
	...
	for (i = 1; i < cnt; i++) {
		...
		if (pmdata->nsvids == 1) {
			...
			atomic_notifier_call_chain(&serenity_chg_usb_tcpm_event_notifier, SPRD_CMD_DISCOVER_SVID, &port->svid_header);
		}
#endif
...
#ifdef SERENITY_FEATURE_CHG_BASIC
	port->svid_header = 0;
	atomic_notifier_call_chain(&serenity_chg_usb_tcpm_event_notifier, SNK_UNATTACHED, &port->svid_header);
#endif
	port->pd_capable = false;
	sprd_tcpm_log(port, "%s:line%d pd_capable flase", __func__, __LINE__);
	port->pps_data.supported = false;
