...
ATOMIC_NOTIFIER_HEAD(serenity_chg_typec_event_notifier);
EXPORT_SYMBOL_GPL(serenity_chg_typec_event_notifier);
int serenity_chg_typec_reg_event_notifier(struct notifier_block *nb)
{
	return atomic_notifier_chain_register(&serenity_chg_typec_event_notifier, nb);
}
EXPORT_SYMBOL_GPL(serenity_chg_typec_reg_event_notifier);

void serenity_chg_typec_unreg_event_notifier(struct notifier_block *nb)
{
	atomic_notifier_chain_unregister(&serenity_chg_typec_event_notifier, nb);
}
EXPORT_SYMBOL_GPL(serenity_chg_typec_unreg_event_notifier);

static u32 usb_online;
bool serenity_chg_typec_get_usb_present(void)
{
	return usb_online;
}
EXPORT_SYMBOL_GPL(serenity_chg_typec_get_usb_present);
#endif
...
#ifdef SERENITY_FEATURE_CHG_BASIC
		usb_online = true;
		atomic_notifier_call_chain(&serenity_chg_typec_event_notifier, POWER_SUPPLY_PROP_ONLINE, &usb_online);
#endif
