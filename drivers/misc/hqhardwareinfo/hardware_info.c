...
#define HARDWARE_INFO_HULK_K "realme Note 50"
#define HARDWARE_INFO_HULK_M "realme C51s"

#define HARDWARE_INFO_LATTE "rmx3930"
#define HARDWARE_INFO_LATTE_A_INDIA "realme C61"
#define HARDWARE_INFO_LATTE_A_NONE_INDIA "realme Note 60"
#define HARDWARE_INFO_LATTE_B "realme C63"
#define HARDWARE_INFO_LATTE_C "realme NARZO N63"
#define HARDWARE_INFO_LATTE_H "realme C61"
#define HARDWARE_INFO_LATTE_K_NONE_INDIA "realme Note 60x"
...
int search_str(char* str, char **result)
{
	...
	printk("[HWINFO] search_str[%s %s]", str, rst);
	*result = rst;
	return 0;
}
EXPORT_SYMBOL(search_str);

static bool is_latte_project(const char *prj_name)
{
    return (strncmp(prj_name, "24661", 5) == 0) || (strncmp(prj_name, "24662", 5) == 0) || (strncmp(prj_name, "24730", 5) == 0) || (strncmp(prj_name, "24717", 5) == 0) ||
           (strncmp(prj_name, "24666", 5) == 0) || (strncmp(prj_name, "24667", 5) == 0) || (strncmp(prj_name, "24668", 5) == 0) || (strncmp(prj_name, "24664", 5) == 0) ||
           (strncmp(prj_name, "24665", 5) == 0) || (strncmp(prj_name, "24729", 5) == 0) || (strncmp(prj_name, "24725", 5) == 0) || (strncmp(prj_name, "24726", 5) == 0) ||
           (strncmp(prj_name, "24734", 5) == 0) || (strncmp(prj_name, "24735", 5) == 0) || (strncmp(prj_name, "24736", 5) == 0) || (strncmp(prj_name, "24731", 5) == 0) ||
           (strncmp(prj_name, "24663", 5) == 0) || (strncmp(prj_name, "24741", 5) == 0) || (strncmp(prj_name, "24737", 5) == 0);
}

static bool is_hulk_project(const char *prj_name)
{
    return (strncmp(prj_name, "22716", 5) == 0) || (strncmp(prj_name, "22717", 5) == 0) || (strncmp(prj_name, "22718", 5) == 0) || (strncmp(prj_name, "22720", 5) == 0) ||
           (strncmp(prj_name, "22721", 5) == 0) || (strncmp(prj_name, "22729", 5) == 0) || (strncmp(prj_name, "22722", 5) == 0) || (strncmp(prj_name, "22723", 5) == 0) ||
           (strncmp(prj_name, "22724", 5) == 0) || (strncmp(prj_name, "22726", 5) == 0) || (strncmp(prj_name, "22734", 5) == 0) || (strncmp(prj_name, "23665", 5) == 0) ||
           (strncmp(prj_name, "23666", 5) == 0) || (strncmp(prj_name, "23711", 5) == 0) || (strncmp(prj_name, "23712", 5) == 0) || (strncmp(prj_name, "23713", 5) == 0) ||
           (strncmp(prj_name, "23714", 5) == 0) || (strncmp(prj_name, "23728", 5) == 0) || (strncmp(prj_name, "23633", 5) == 0);
}
...
static int hq_prj_name_setup()
{
	...
	search_str("sprdboot.marketing.name=", &market_name);
	if (market_name == NULL) {
		return -1;
	}
	memset(market, 0, sizeof(market));
	memcpy(market, market_name, strlen(market_name));
	...
	if ((strncmp(prj_name, "22716", 5) == 0) || (strncmp(prj_name, "22717", 5) == 0) ||
		...
		(strncmp(prj_name, "23713", 5) == 0) || (strncmp(prj_name, "23714", 5) == 0) ||
		(strncmp(prj_name, "23728", 5) == 0)){
		prj_name_flag = 7;
	} else if (strncmp(prj_name, "23633", 5) == 0){
		prj_name_flag = 8;
	}

	if (strncmp(market, "other", 5) == 0) {
		if ((strncmp(prj_name, "24661", 5) == 0) || (strncmp(prj_name, "24662", 5) == 0) || (strncmp(prj_name, "24717", 5) == 0)) {//LATTE
			prj_name_flag = 1;
		} else if (strncmp(prj_name, "24663", 5) == 0) {//LATTE_A_INDIA
			prj_name_flag = 2;
		} else if ((strncmp(prj_name, "24666", 5) == 0) || (strncmp(prj_name, "24667", 5) == 0) ||(strncmp(prj_name, "24668", 5) == 0)) {//LATTE_B
			prj_name_flag = 3;
		} else if ((strncmp(prj_name, "24664", 5) == 0) || (strncmp(prj_name, "24665", 5) == 0) ||(strncmp(prj_name, "24729", 5) == 0)) {//LATTE_A_NONE_INDIA
			prj_name_flag = 6;
		} else if ((strncmp(prj_name, "24725", 5) == 0) || (strncmp(prj_name, "24726", 5) == 0) ||(strncmp(prj_name, "24734", 5) == 0)
		|| (strncmp(prj_name, "24735", 5) == 0 || (strncmp(prj_name, "24736", 5) == 0))) {//LATTE_K_NONE_INDIA
			prj_name_flag = 7;
		}
	} else if ((strncmp(prj_name, "24666", 5) == 0) && (strncmp(market, "latte-C", 7) == 0)) {//LATTE_C
		prj_name_flag = 4;
	} else if ((strncmp(prj_name, "24668", 5) == 0) && (strncmp(market, "latte-H", 7) == 0)) {//LATTE_H
		prj_name_flag = 5;
	}
	return 0;
}
...
static ssize_t show_version(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if ((strncmp(prj_name, "24661", 5) == 0) || (strncmp(prj_name, "24662", 5) == 0) || (strncmp(prj_name, "24730", 5) == 0) || (strncmp(prj_name, "24717", 5) == 0) \
    || (strncmp(prj_name, "24666", 5) == 0) || (strncmp(prj_name, "24667", 5) == 0) || (strncmp(prj_name, "24668", 5) == 0) || (strncmp(prj_name, "24664", 5) == 0) \
    || (strncmp(prj_name, "24665", 5) == 0) || (strncmp(prj_name, "24729", 5) == 0) || (strncmp(prj_name, "24725", 5) == 0) || (strncmp(prj_name, "24726", 5) == 0) \
    || (strncmp(prj_name, "24734", 5) == 0) || (strncmp(prj_name, "24735", 5) == 0) || (strncmp(prj_name, "24736", 5) == 0) || (strncmp(prj_name, "24731", 5) == 0) \
    || (strncmp(prj_name, "24663", 5) == 0) || (strncmp(prj_name, "24741", 5) == 0) || (strncmp(prj_name, "24737", 5) == 0)) {
    ...
    } else if ((strncmp(prj_name, "22716", 5) == 0) || (strncmp(prj_name, "22717", 5) == 0) || (strncmp(prj_name, "22718", 5) == 0) || (strncmp(prj_name, "22720", 5) == 0) \
    || (strncmp(prj_name, "22721", 5) == 0) || (strncmp(prj_name, "22729", 5) == 0) || (strncmp(prj_name, "22722", 5) == 0) || (strncmp(prj_name, "22723", 5) == 0) \
    || (strncmp(prj_name, "22724", 5) == 0) || (strncmp(prj_name, "22726", 5) == 0) || (strncmp(prj_name, "22734", 5) == 0) || (strncmp(prj_name, "23665", 5) == 0) \
    || (strncmp(prj_name, "23666", 5) == 0) || (strncmp(prj_name, "23711", 5) == 0) || (strncmp(prj_name, "23712", 5) == 0) || (strncmp(prj_name, "23713", 5) == 0) \
    || (strncmp(prj_name, "23714", 5) == 0) || (strncmp(prj_name, "23728", 5) == 0) || (strncmp(prj_name, "23633", 5) == 0)) {
...
static ssize_t show_nfc(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    search_str("prj_name=", &nfc_sku);
    printk("nfc sku =%s\n",nfc_sku);

    if (NULL != hwinfo_data.nfc)
    printk("nfc chip type%s\n",hwinfo_data.nfc);


    if((strcmp(nfc_sku,"24668") != 0) &&
       (strcmp(nfc_sku,"24662") != 0) &&
       (strcmp(nfc_sku,"24730") != 0) ){
        return sprintf(buf, "nfc name :Not support\n");
    }

    if (NULL != hwinfo_data.nfc) {
        return sprintf(buf, "nfc chip_hwid :%s\n", hwinfo_data.nfc);
    } else {
        return sprintf(buf, "nfc name :UNKONW\n");
    }
}
