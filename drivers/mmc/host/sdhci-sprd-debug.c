...
static bool remove_card;

/* Store anomaly information of mmc every 10 seconds */
struct sd_anomaly_info {
	u32 rd_err_times; /* durations(ms) of data read timeout error occurrance */
	u32 rd_rsp_over_times; /* durations(ms) of those read response over half second*/
	u32 wr_err_times; /* durations(ms) of data write timeout error occurrance */
	u32 wr_rsp_over_times; /* durations(ms) of those write response over half second*/
};
...
static void mmc_debug_info_print(struct sdhci_host *host, struct mmc_debug_info *info)
{
	...
	if (info->read_total_time || info->write_total_time)
		pr_err("|__time%8s: r= %ldms, w= %ldms\n",
			name, info->read_total_time / 1000, info->write_total_time / 1000);
	if (info->ano_info.rd_err_times || info->ano_info.wr_err_times)
		pr_err("|__timeout%5s: r= %dms, w= %dms\n",
			name, info->ano_info.rd_err_times, info->ano_info.wr_err_times);
	if (info->ano_info.rd_rsp_over_times || info->ano_info.wr_rsp_over_times)
		pr_err("|__slw_rsp%5s: r= %dms, w= %dms\n",
			name, info->ano_info.rd_rsp_over_times, info->ano_info.wr_rsp_over_times);
}
...
static void sd_anomaly_check_handle(struct mmc_debug_info *info, struct sdhci_host *host)
{
	...
	u64 rspeed, u64 wspeed)
{
	...
	u32 anomaly_durations;
	u64 total_time;
	u32 level = 0; /* anomaly level */
	u32 polling_time = sdhci_sprd_mmc_update_polling_times() * 1000; /*ms*/
	...
	anomaly_durations = info->ano_info.rd_rsp_over_times + info->ano_info.wr_rsp_over_times;
	if (anomaly_durations && anomaly_durations < polling_time / 4) {
		level += 1;
		strcat(ecode, "1");
	} else if (anomaly_durations >= polling_time / 4) {
		level += 2;
		strcat(ecode, "2");
	} else
		strcat(ecode, "0");
	...
	if ((info->read_total_blocks > SD_VALID_CALC_CNT || info->ano_info.rd_err_times ||
			info->ano_info.rd_rsp_over_times) && rspeed < SD_SPEED_LIMIT) {
		level += 1;
		strcat(ecode, "1");
	} else
		strcat(ecode, "0");
	...
	if ((info->write_total_blocks > SD_VALID_CALC_CNT || info->ano_info.wr_err_times ||
			info->ano_info.wr_rsp_over_times) && wspeed < SD_SPEED_LIMIT) {
		level += 1;
		strcat(ecode, "1");
	} else
		strcat(ecode, "0");
	...
	if (total_time / 1000 >= polling_time * 9 / 10) {
		level += 1;
		strcat(ecode, "1");
	} else
		strcat(ecode, "0");
	...
	if (level >= ANOMALY_WARNING_LEVEL) {
		...
		pr_err("%s: identify anomaly level %d, %s\n",
		...
		if (card) {
			...
			pr_info("%s: %s%scard; card capacity: 0x%x; card manfid: 0x%x\n",
				mmc_hostname(host->mmc),
				mmc_card_uhs(card) ? "ultra high speed " :
				(mmc_card_hs(card) ? "high speed " : ""),
				uhs_bus_speed_mode, card->csd.capacity, card->cid.manfid);
		}
		if (level >= ANOMALY_REMOVED_LEVEL) {
			/* Card would be removed after two continuous anomaly periods */
			if (remove_card) {
				sd_anomaly_check_handle(info, host);
			} else {
				remove_card = true;
				return;
			}
...
static void mmc_debug_handle_rsp(struct sdhci_host *host, struct mmc_debug_info *info)
{
	...
	if (info->intmask & SDHCI_INT_DATA_END || info->intmask & SDHCI_INT_DATA_TIMEOUT) {
		...
		if (info->intmask & SDHCI_INT_DATA_TIMEOUT) {
			info->blocks = 0;
			if (rw == READ_TYPE)
				info->ano_info.rd_err_times += msecs;
			else if (rw == WRITE_TYPE)
				info->ano_info.wr_err_times += msecs;
		} else if (index >= 10) {
			if (rw == READ_TYPE)
				info->ano_info.rd_rsp_over_times += msecs;
			else if (rw == WRITE_TYPE)
				info->ano_info.wr_rsp_over_times += msecs;
