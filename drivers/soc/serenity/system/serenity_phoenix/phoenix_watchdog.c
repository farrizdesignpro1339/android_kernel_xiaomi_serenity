...
//#include <soc/serenity/system/boot_mode.h>
//#include <soc/serenity/system/serenity_project.h>
...
static int hang_serenity_main_on = 1; /* default on */
static int hang_serenity_recovery_method = RESTART_AND_RECOVERY;
...
static void phx_pet(void)
{
	...
	if (is_userdebug || g_is_aging_version) {
		PHX_KLOG_INFO("will not set ERROR_HANG_SERENITY with userdebug/aging version\n");
		return;
	}

	PHX_KLOG_INFO("phoenix watchdog pet!\n");
	phx_set_boot_error(ERROR_HANG_SERENITY);
	return;
}
...
/*
	...
	if (str) {
		...
		get_option(&str, &hang_serenity_main_on);
		PHX_KLOG_INFO("hang_serenity_main_on %d\n", hang_serenity_main_on);
	}
*/
	hang_serenity_main_on = 1;
	PHX_KLOG_INFO("hang_serenity_main_on %d\n", hang_serenity_main_on);
...
/*	str = strstr(bootargs_ptr, "phx_rus_conf.recovery_method=");
	if (str) {
		...
		get_option(&str, &hang_serenity_recovery_method);
		PHX_KLOG_INFO("hang_serenity_recovery_method %d\n",
			      hang_serenity_recovery_method);
	}
*/
	hang_serenity_recovery_method=2;
	PHX_KLOG_INFO("hang_serenity_recovery_method %d\n",hang_serenity_recovery_method);
...
void __init phoenix_hlos_watchdog_init(void)
{
	...
	PHX_KLOG_INFO("phoenix hlos watchdog: %s\n",
		      hang_serenity_main_on ? "on" : "off");
	if (hang_serenity_main_on && phx_is_normal_mode()) {
