...
#define FILE_DMESG "/data/persist_log/serenityreserve/phoenix/kmsg"
#define PHOENIX_TEMP_DIR "/data/persist_log/serenityreserve/phoenix"
#define FILE_KERNEL_EXPETION_INFO                                              \
	"/data/persist_log/serenityreserve/phoenix/ERROR_KERNEL_EXCEPTION"
...
extern int hang_serenity_recovery_method;
extern int hang_serenity_main_on;
...
static void phx_dump_hang_serenity_log(const char *);
...
static phx_action_mapping phx_log_dumper_mapping[] = {
	{ ERROR_HANG_SERENITY, phx_dump_hang_serenity_log },
...
static void phx_dump_hang_serenity_log(const char *happen_time)
{
	struct task_struct *t_init;
	t_init = NULL;
	find_task_by_comm(TASK_INIT_COMM, &t_init);
	if (NULL != t_init && phx_is_filesystem_ready()) {
		PHX_KLOG_INFO("send hang serenity signal %d at %s", SIGPHX_HANG,
			      happen_time);
...
#if IS_MODULE(CONFIG_SERENITY_FEATURE_PHOENIX)
