/***************************************************************
** Copyright (C),  2019-2020,  SERENITY Mobile Comm Corp.,  Ltd
** VENDOR_EDIT
** File : uboot_log.c
...
#include <soc/serenity/system/hung_task_enhance.h>
#include <soc/serenity/system/serenity_signal.h>

#include <linux/version.h>

#ifdef CONFIG_SERENITY_FEATURE_DEATH_HEALER
/*
...
static void serenity_check_hung_task(struct task_struct *t, unsigned long timeout, unsigned int *iowait_count, bool *show_lock, bool *call_panic)
{
...
/* DeathHealer, kill D/T/t state tasks */
	...
	if (switch_count != t->last_switch_count) {
		...
		#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)) || defined(CONFIG_SERENITY_SYSTEM_KERNEL_QCOM)
...
void io_check_hung_detection(struct task_struct *t, unsigned long timeout, unsigned int *iowait_count, bool *show_lock, bool *call_panic)
{
	...
	if (t->state == TASK_UNINTERRUPTIBLE || t->state == TASK_STOPPED || t->state == TASK_TRACED)
		...
		if (!sysctl_hung_task_selective_monitoring ||
			...
			serenity_check_hung_task(t, timeout, iowait_count, show_lock, call_panic);
#endif
	return;
}
