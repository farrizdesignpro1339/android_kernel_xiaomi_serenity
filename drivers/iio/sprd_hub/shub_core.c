...
static bool check_prj_name_hulk(char *prj_name)
{
	bool is_HULK = false;
	if (prj_name !=NULL && (strncmp(prj_name,"23711",5) ==0 || strncmp(prj_name,"23712",5) ==0
			...
			|| strncmp(prj_name,"23728",5) ==0 || strncmp(prj_name,"23633",5) ==0
			|| strncmp(prj_name,"22734",5) ==0 || strncmp(prj_name,"22726",5) ==0
			|| strncmp(prj_name,"22722",5) ==0 || strncmp(prj_name,"22723",5) ==0
			|| strncmp(prj_name,"22724",5) ==0 || strncmp(prj_name,"22716",5) ==0
			|| strncmp(prj_name,"22717",5) ==0 || strncmp(prj_name,"22718",5) ==0  )) {
		is_HULK = true;
        }
	return is_HULK;
}
...
static void sensor_get_prj_name(char *buf, size_t len)
{
	...
	if (check_prj_name_hulk(temp_name)) {
...
static int sensor_set_nfc_flag(struct shub_data *sensor)
{
	int err = 0;
	...
	printk("%s: prj_name=%s\n", __func__, prj_name);
	if (!check_prj_name_hulk(prj_name)) {
	 	printk("Only Hulk need ,others skip\n");
	 	return err;
	}
	...
	printk("mag hulk get prj_nfc_flag = %s  \n",prj_name);
	printk("mag hulk err = %d	\n",err);
	if (err < 0) {
		printk("lks_mag sensor_set_nfc_flag send command Fail\n");
		return err;
	}
	return 0;
}
