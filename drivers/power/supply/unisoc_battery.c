...
#define SERENITY_FEATURE_CHG_BASIC 1
struct sprd_fgu_data *gauge_data;

#ifdef SERENITY_FEATURE_CHG_BASIC
#include "../serenity/v1/serenity_gauge.h"
#endif
...
extern void serenity_gauge_init(struct serenity_gauge_chip *chip);
extern int serenity_chg_get_voocphy_support(void);
extern int serenity_force_get_subboard_temp(void);
...
#ifdef SERENITY_FEATURE_CHG_BASIC
#define SERENITY_BATON_NTC_01 10
#endif
...
int serenity_battery_get_bat_temperature(void)
{
	if (is_battery_init_done()) {
		//return (force_get_tbat(serenity_gm, true) * 10);
		return SERENITY_TEMP_25;
	} else {
		return -1270;
	}
}
...
static struct serenity_gauge_operations serenity_battery_gauge = {
...
{
#ifdef SERENITY_FEATURE_CHG_BASIC
	struct serenity_gauge_chip *chip = NULL;
#endif
...
#ifdef SERENITY_FEATURE_CHG_BASIC
	if(is_fuelgauge_apply() == true) {
		chip = (struct serenity_gauge_chip*) kzalloc(sizeof(struct serenity_gauge_chip),
					GFP_KERNEL);
		if (!chip) {
			bm_err("serenity_gauge_chip devm_kzalloc failed.\n");
			return -ENOMEM;
		}
	}
	if(is_fuelgauge_apply() == true) {
		chip->gauge_ops = &serenity_battery_gauge;
		serenity_gauge_init(chip);
