 ...
 */
#ifndef SERENITY_PROJECT_DATA_OCDT_H
#define SERENITY_PROJECT_DATA_OCDT_H
...
#ifdef CONFIG_SERENITY_SYSTEM_KERNEL_QCOM
enum PCB_VERSION {
    UNKNOWN_VERSION = 0,
    PRE_EVB1,
    PRE_EVB2,
...
enum {
    ...
    REBOOT_MODEM        = 0x28,
    REBOOT_ANDROID      = 0x29,
//Modify for PHOENIX_PROJECT and SERENITY_DOCTOR use
...
/*
serenity board feature mask which value read from ocdt.bin design to eliminate
use of get_project.
One byte for one feature, so one feature support 256 different status
*/
typedef enum {
	SERENITY_BOARD_FEATURE_VIBRATOR_STATUS = 0,  /*add as a example */
	SERENITY_BOARD_FEATURE_END = FEATURE_COUNT,
} SerenityBoardFeatureMask;
